...
Initial results are very promising with the tf.idf weighted cosine providing much more accurate results than with simple euclidean distance.  However, there is not a large enough truth set of plot similarities to be able to confidently say that it is effective.  Nevertheless these promising early results seem to indicate that further research would certainly be worthwhile, but this is outside the scope of the project.

\tdi{I'm planning on doing a little bit of investigation here so that the results will be drawn a corpus with more than 10 plots.}
