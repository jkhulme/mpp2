...
\begin{itemize}
\item There was inconsistency in the text giving instructions to the user in places
\item The users kept trying to drag annotations on the graph after creation
\item Model visualisation in session wizard should have species labels
\item Thought the appearance was good -- This is with the addition of the annotation animation panel on the right hand side.
\item Users thought the task was easy to complete.
\item Users found the new session wizard much easier to use
\item Users found the new model visualisation more intuitive
\item Users found annotating the animation useful, Users found it easy to add annotations
\item The users uncovered some bugs in the program
\item Very much in favour of plots as queries
\item Very much in favour of collaboration
\item Suggested they would much rather the program be more stable before new features are added
\item Very easy and useful to normalise and export data
\item Found being able to undo very useful
\item Pleased to be able to modify or delete the annotations
\end{itemize}

\subsection{Bio-PEPA Developer}

\begin{itemize}
\item Didn't realise what the model visualisation was saying
\item Uncovered a flaw with annotations and normalisation
\item Had no significant difficulty
\item Bug when trying to annotate outside of graph area, but in the frame.
\end{itemize}
...
Still too many bugs
animation annotation much more user friendly
Please about reception to ideas of collaboration and search.
...
\begin{itemize}
\item The users like the appearance of the tool
\item The users preferred the new layout.
\item The users found it possible to complete the task
\item Both users indicated they would rather use this than the Eclipse plugin (proviso, they do not use the eclipse plugin, their experience with it is very limited)
\item Both indicated they would be willing to use it in their workflow
\item Were not able to give feedback on missing features as they had not used the tool for long enough, or in real enough circumstances.
\item Same on any features they would like to see in the future
\item Were confused when annotation wasn't present after setting the start time to be after 0.
\item slowness of the tool at times caused confusion - clicking things multiple times
\item model visualisation was more useful to them with labels
\item text was helpful in ui elements
\end{itemize}

\subsection{Bio-PEPA developer}

\begin{itemize}
\item Thought the appearance of the tool was average.  There was a quality on the tip of their tongue.  They did not have anything negative to say.  More headings in sections.
\item Much preferes the new interface
\item Found it difficult to complete the task, but easier than previous times.  The speed did not help.  More of a help functionality
\item Would use the tool rather than the eclipse plugin, but depends on the task.  Preparing a presentation - this, analysing models - eclipse
\item Would like model visualisation to not necessarily be heirarchical, problem with Bio-PEPA.  Could be looked at in future.
\item Would have liked to have evaluated the tool with more models -- ethnographic.
\end{itemize}

\subsection{Personal}

Pleased that fewer bugs discovered - a promising sign
New user interface is good
Good sign that the users would be willing to use the tool instead of eclipse
valid point about evaluation, discussed later.
